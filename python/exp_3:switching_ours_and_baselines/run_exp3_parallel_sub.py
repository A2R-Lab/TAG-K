        ...
        for k in range(len(t)):
            ...
            if (k % est_freq == 0) and (k > 0):
                ...
                if estimator_name == "gt":
                    ...
                    used_estimator = est
                    # during substitution step (each estimate epoch):
                    if sub_active and sub_est is not None:
                        # run partner update
                        theta_est = sub_est.iterate(A_stack, b_stack)
                        # keep baseline “alive”
                        if hasattr(est, "predict_only"):
                            est.predict_only(steps=1)

                    # when closing substitution:
                    if sub_left <= 0:
                        th_sub = get_theta_vector(sub_est)
                        if hasattr(est, "inject_theta"):
                            est.inject_theta(th_sub, K=sub_k, beta=1e-3)   # tune beta if needed
                        else:
                            set_theta_vector(est, th_sub)
