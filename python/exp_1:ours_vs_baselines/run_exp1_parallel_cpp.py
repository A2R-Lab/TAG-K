...
from online_estimation import _core
import time
...
def make_estimator(name: str, theta0: np.ndarray, window: int):
    ...
    if name == "rls_0.99":
        ...
        return _core.RLS(n, x0=theta0, **rls_kwargs)
    if name == "rls_0.96":
    ...
    if name == "kf_low":
        ...
        return _core.KF(n, process_noise=Q, measurement_noise=Rm, x0=theta0, p_coeff=10)
    if name == "kf_high":
        Q = 1e-1 * np.eye(n)
    ...
    if name == "tagrk":
        ...
        return _core.TAGRK(n, x0=theta0, burnin_steps=6, tolerance=1e-9)
    if name in ("gt", "none"):
        return None
...
def run_single_trial(
    ...
    *,
        ...
        for k in range(len(t)):
            ...
            if (k % est_freq == 0) and (k > 0):
                ...
                if estimator_name == "gt":
                    ...
                    theta_est = est.iterate(A_stack, b_stack).flatten()
                    time_elapsed = time.process_time() - start_time
                    solver_times.append(time_elapsed)
    ...
    times_arr  = np.full(M_EST, np.nan)
    times_arr[:min(len(solver_times), M_EST)] = np.asarray(solver_times)[:M_EST]
    ...
    return {
        ...
        "rng_seed": int(base_seed + seed_offset),
        "solver_times": times_arr
    }
...
def main():
    ...
    store: Dict[str, Dict[str, List[np.ndarray] | None]] = {
        algo: {
            ...
            "theta_gt": [], "theta_est": [], "A": [], "b": [], "solver_times": []
        }
        for algo in ALGOS
    }
    ...
    for ref_type in ref_types:
        ...
        for a_idx, algo in enumerate(ALGOS):
            ...
            for idx, out in enumerate(results):
                ...
                store[algo]["solver_times"].append(out["solver_times"]) # type: ignore
                if args.save_traj:
    ...
    for algo in ALGOS:
        ...
        save_kwargs[f"{algo}__times"] = np.stack(store[algo]["solver_times"], axis=0)
